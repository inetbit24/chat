...
<div id="chat-form" class="chat1">
	<div class="chat2">
		<div class="chat3"></div>
        <div class="chat4">
        	<div id="winchat" class="chat5">

            <template>
              <v-infinite-scroll :loading="loading" @top="prevPage" @bottom="nextPage" :offset='4' style="max-height: 477px; overflow-y: scroll;">
                <div class="message5" v-for="(item, index) in items" v-bind:key="item.login">
                    <div class="message">
                    	<div class="message1">{{item.login}}</div>
                    	<div class="message2">
                        	{{item.message}}
                            <div class="message3">{{item.rdate}}</div>
                        	<div class="message4" @click="deleteMessage(index)">
        ...
        <div class="chat6">
            <div class="chat7">
                <div class="chat8">
                	<input type="text" value="" v-model="message" placeholder="Сообщение">
                </div>
                <div class="chat9">
                	<button class="chat10" @click="addMessage"><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M21.0187 10.2213L1.76874 0.596256C1.61786 0.520801 1.44839 0.490569 1.28073 0.509197C1.11307 0.527826 0.954373 0.594523 0.82374 0.701255C0.698987 0.805812 0.605871 0.943094 0.554861 1.09767C0.503851 1.25224 0.49697 1.41798 0.53499 1.57626L3.12499 11L0.49999 20.3975C0.464314 20.5297 0.460149 20.6684 0.487832 20.8025C0.515514 20.9365 0.574272 21.0622 0.65938 21.1695C0.744487 21.2767 0.85357 21.3625 0.977857 21.4199C1.10214 21.4773 1.23817 21.5047 1.37499 21.5C1.51197 21.4992 1.64683 21.4662 1.76874 21.4038L21.0187 11.7788C21.1621 11.7053 21.2824 11.5938 21.3663 11.4564C21.4503 11.319 21.4948 11.161 21.4948 11C21.4948 10.839 21.4503 10.681 21.3663 10.5436C21.2824 10.4062 21.1621 10.2947 21.0187 10.2213ZM2.73124 18.9713L4.66499 11.875H12.75V10.125H4.66499L2.73124 3.02876L18.665 11L2.73124 18.9713Z" fill="#0047FF"/>
    ...
    <div class="login1">
    	<div class="login2">
        	<div class="login3">{{input_login}}</div>
        	<div class="login4"><input type="text" value="" v-model="login" placeholder="Логин"></div>
        </div>
    </div>
</div>
